/*the controls are slightly confusing;

RIGHT = you can go through lasers(the blue rects) you turn orange...
LEFT = a small jump
DOWN = you shrink
UP = big jump!
each obstacle uses its own keyIsPressed function.
have fun!
...
var new_rect = function(x, y, w, h, c, l) {
    var r = new Object(1);
    r.x = x;
    r.y = y;
    r.w = w;
...
var new_barrier = function (t, x) {
    ...
    switch (t) {
        case 1:
            b.r.push(new_rect (x, base_y, rect_width, -rect_hight,[156, 121, 69],0));
            b.r.push(new_rect (x, base_y -2.3*rect_hight, rect_width, -rect_hight,[156, 121, 69],0));

            break;
        case 2:
            b.r.push(new_rect (x, base_y, rect_width, -rect_hight*2,[156, 121, 69] , 0));
            break;
        case 3:
            b.r.push(new_rect (x, base_y-15, rect_width, -3*rect_hight,[156, 121, 69] ,0));
            break;
        case 4:
            for (var i=0; i<25; i++) {

                b.r.push (new_rect (x, base_y-i*10, rect_width, -5,[i*10, 255 - i*10, 255 + i*10],1));
        ...
        case 5:
            for (var i=0; i<25; i++) {
                ...
                b.r.push (new_rect (x, base_y-i*10, rect_width, -5,[0,0,0],1));
    ...
    for(var i = 0 ; i < barriers.length ;  i += 1){
        ...
        for (var j=0; j < barriers[i].r.length; j++) {
            ...
            if (barriers[i].r[j].overlaps (guy)||guy.overlaps(barriers[i].r[j])) {
                if (barriers[i].r[j].l === 0||!(laser_on)) {
                    dead = true;
                }
            }
        }
