...
var t = [];
var tower = function(t, c, d, r, x, y){
    var t = new Object(1);
    t.t = t;
    t.c = c;
    t.d = d;
...
var run_game = function(){
    ...
    for(var i = 0; i < t.length; i ++){
        if(t[i].e === true){
            ...
            noFill();
            stroke(255, 0, 0);
            ellipse(t[i].x,t[i].y,t[i].r,t[i].r);
            stroke(0, 0, 0);
            fill(110, 110, 110);
        }
        fill(110, 110, 110);
        ellipse(t[i].x,t[i].y,t[i].d,t[i].d);
    }                  //tower edit
    for(var i = 0; i < b.length; i ++){
        if(b[i].l === true){
    ...
    for(var i = 0; i < t.length; i ++){
        ...
        if(t[i].t === 1 && t[i].e === false){
            if(t[i].c > 50){
                bestnum = -1;
                for(var j = 0; j < z.length; j ++){
                    if(sqrt(sq(z[j].x - t[i].x) + sq(z[j].y - t[i].y)) < t[i].r && z[j].l === true){
                        if(sqrt(sq(z[j].x - t[i].x) + sq(z[j].y - t[i].y)) < bestdis){
                            bestdis = sqrt(sq(z[j].x - t[i].x) + sq(z[j].y - t[i].y));
                            bestnum = j;
                        }
                    }
                    t[i].c = 0;
                ...
                if (bestnum !== -1){
                    b.push(bullet(1, 1, 5, 3, t[i].x, t[i].y, z[bestnum].x, z[bestnum].y));
                    bestdis = 10000;
                    bestnum = 10000;
                }
            }
        }
        if(t[i].t === 2 && t[i].e === false){
            if(t[i].c > 5){
                bestnum = -1;
                for(var j = 0; j < z.length; j ++){
                    if(sqrt(sq(z[j].x - t[i].x) + sq(z[j].y - t[i].y)) < t[i].r && z[j].l === true){
                ...
                if (bestnum !== -1){
                    b.push(bullet(1, 0.2, 1, 5, t[i].x, t[i].y, z[bestnum].x, z[bestnum].y));
                    bestdis = 10000;
                    bestnum = 10000;
                }
            }
        }
        if(t[i].t === 3 && t[i].e === false){
            if(t[i].c > 50){
                bestnum = -1;
                for(var j = 0; j < z.length; j ++){
                    if(sqrt(sq(z[j].x - t[i].x) + sq(z[j].y - t[i].y)) < t[i].r && z[j].l === true){
                ...
                if (bestnum !== -1){
                    for(var j = 0; j < 5; j ++){
                        b.push(bullet(1, 0.7, random(1,2), 3.5, t[i].x, random(t[i].y,t[i].y + 5), random(z[bestnum].x - 10,z[bestnum].x + 10), random(z[bestnum].y - 10,z[bestnum].y + 10)));
                    }
                    bestdis = 10000;
                    bestnum = 10000;
                }
        ...
        if(t[i].t === 4&& t[i].e === false){
            if(t[i].c > 200){
                ...
                if (bestnum !== -1){
                    b.push(bullet(1, 10, 2, 10, t[i].x, t[i].y, z[bestnum].x, z[bestnum].y));
                    bestdis = 10000;
                    bestnum = 10000;
        ...
        if(t[i].t === 5 && t[i].e === false){
            if(t[i].c < -3){
                b.push(bullet(2, 0.005, 50, 5, t[i].x, t[i].y, 0, 0, t[i].x, t[i].y));
                bestdis = 10000;
                bestnum = 10000;
            }
...
mouseClicked = function(){
    if(draw === upgrades){
        if(sqrt(sq(mouseX - 50) + sq(mouseY - 50)) < 20){
            t.push(tower(1,1,20,150,mouseX,mouseY));
            draw = run_game;
        }
        if(sqrt(sq(mouseX - 50) + sq(mouseY - 100)) < 20){
            t.push(tower(2,1,20,100,mouseX,mouseY));
            draw = run_game;
        }
        if(sqrt(sq(mouseX - 50) + sq(mouseY - 150)) < 20){
            t.push(tower(3,1,20,75,mouseX,mouseY));
            draw = run_game;
        }
        if(sqrt(sq(mouseX - 50) + sq(mouseY - 200)) < 20){
            t.push(tower(4,1,20,300,mouseX,mouseY));
            draw = run_game;
        }
        if(sqrt(sq(mouseX - 50) + sq(mouseY - 250)) < 20){
            t.push(tower(5,-5,20,100,mouseX,mouseY));
            draw = run_game;
        }
